...
def checks_df(result: core.WorkflowResult | list[core.CheckItem]) -> pd.DataFrame:
    checks = result.checks if isinstance(result, core.WorkflowResult) else result
    return pd.DataFrame([asdict(item) for item in checks]).rename(
...
def fetch_failure_checks(alpha_key: str, error: str) -> list[core.CheckItem]:
    ...
    return [
        ...
        core.CheckItem("SKIP", "動能公式計算", "本次未執行，因 QQQ / TLT 缺少可用價格資料。"),
        core.CheckItem("SKIP", "紀錄寫入", "本次未寫入 signals.csv，保留上一次成功計算紀錄。"),
    ]
...
def main() -> None:
    ...
    if st.session_state.page == "使用說明":
        ...
        st.markdown(
            ...
            **這個程式在做什麼**：本工具用 QQQ 與 TLT 的調整後月收盤價，計算 1 個月、3 個月、6 個月平均動能，產生建議標的與是否換倉。

            **公式邏輯**：以基準月價格分別對比 1 個月前、3 個月前、6 個月前價格，三個報酬率平均後得到動能分數。例如正式計算 2026-05-01，基準月是 2026-04，對比 2026-03、2026-01、2025-10。

            **上方按鈕**：
            - **正式計算**：只使用已完成月 K，結果會寫入 `signals.csv`，適合每月正式檢查與換倉。
            - **月底預估**：用本月目前價格暫代本月月底月 K，提前預估下一個執行月；畫面會標示「月底預估」，不寫入正式紀錄。
            - **首頁**：回到主計算畫面。
            - **使用說明**：切換到本頁。

            **資料來源與 API Key**：Yahoo Finance 是主要來源，不需 API Key。Alpha Key 為選填；輸入後會在背景比對 Yahoo 與 Alpha Vantage 價格差異。只有 Yahoo 與本地快取都缺少必要月份時，才會用 Alpha 備援；沒有輸入 Key 時，會略過比對與備援。

            **正式計算**：依 `signals.csv` 最後一筆正式紀錄推算下一期，只使用已完成月 K。基準月尚未結束時會顯示黃色「尚未到正式計算時間，等待 X 月完成」；這是時間提醒，不抓價、不計算、不寫入。基準月完成後的下一個日曆月第一天起可執行；即使月初沒有立刻按，之後仍可補算尚未寫入的正式期。

            **月底預估**：可在整個月內使用本月目前價格暫代月底月 K。若本月 Yahoo、快取與 Alpha 都無本月價格，會顯示紅色異常，不會使用舊月價格替代。

            **失敗保護**：Yahoo 暫時失敗時會先用本地快取；快取不足且有 Alpha Key 才會嘗試 Alpha。三者都無法提供必要月份時，不會計算或寫入紀錄，會顯示紅色「價格抓價異常：無法判斷動能」。
            ...
            **快取保護**：5 分鐘內重複按相同計算，會優先使用本地快取，避免一直線上抓價。超過 5 分鐘才會重新嘗試線上抓價。Alpha 免費方案可能短暫限流；此時會保留可用快取並提示，稍候再按一次即可重試。
    ...
    if not result:
        ...
        if st.session_state.get("run_error"):
            st.error("本次無可用價格資料，未執行公式計算或寫入紀錄。")
            st.markdown('<div class="section-title">檢查項目</div>', unsafe_allow_html=True)
            st.dataframe(
                style_checks(checks_df(fetch_failure_checks(st.session_state.get("alpha_key", ""), st.session_state.run_error))),
                hide_index=True,
    ...
    if st.session_state.get("run_error"):
        st.error("本次無可用價格資料，未執行公式計算或寫入紀錄；下方顯示的是上一次成功結果。")
        display_checks = fetch_failure_checks(st.session_state.get("alpha_key", ""), st.session_state.run_error) + result.checks
    ...
    with top_right:
        st.markdown('<div class="section-title">檢查項目</div>', unsafe_allow_html=True)
        st.dataframe(style_checks(checks_df(display_checks)), hide_index=True, use_container_width=True, height=205)
