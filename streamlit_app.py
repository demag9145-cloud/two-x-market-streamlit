...
def main() -> None:
    ...
    if st.session_state.page == "使用說明":
        ...
        st.markdown(
            ...
            **資料來源**：Yahoo Finance 是主要來源，不需 API Key。Alpha Key 為選填；輸入後會在背景比對，只有 Yahoo 與本地快取都缺少必要月份時才會用 Alpha 備援。

            **正式計算**：依 `signals.csv` 最後一筆正式紀錄推算下一期，只使用已完成月 K。基準月尚未結束時會顯示黃色等待，不抓價、不計算、不寫入；結束後的下一個日曆月第一天起可執行，結果會寫入正式紀錄。

            **月底預估**：使用本月目前價格暫代月底月 K，提前預估下一個執行月，不寫入正式紀錄。

            **失敗保護**：Yahoo 暫時失敗時會先用本地快取；快取不足才會嘗試 Alpha。三者都無法提供必要月份時，不會計算或寫入紀錄，會顯示紅色異常。
            ...
            **快取保護**：5 分鐘內重複按相同計算，會優先使用本地快取，避免一直線上抓價。Alpha 免費方案可能短暫限流；此時會保留可用快取並提示，稍候再按一次即可重試。
