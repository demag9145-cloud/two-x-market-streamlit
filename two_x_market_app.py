...
class TwoXMarketApp:
    def __init__(self, root: Tk) -> None:
        ...
        self.signal_heading = StringVar(value="建議標的：")
...
【正式週期與資料不足】
正式計算會依 signals.csv 最後一筆正式紀錄推算下一期，不會只看目前畫面或快取的最新月份。

若下一期的基準月尚未完成，會顯示黃色「尚未到正式計算時間，等待 X 月完成」。這是時間提醒，不是錯誤；本次不抓價、不計算，也不寫入 signals.csv。

基準月完成後的下一個日曆月第一天起，才能進行該期正式計算；正式訊號日期會使用該月第一個美股交易日。即使月初沒有立刻按，之後仍可依 signals.csv 補算尚未寫入的正式期。

Yahoo 線上抓價暫時失敗時，程式會先使用本地 Yahoo 快取；若快取沒有本次公式必需月份，且已輸入 Alpha Key，才會嘗試 Alpha Vantage 備援。

若 Yahoo、可用快取與 Alpha 備援都無法提供本期必要資料，會顯示紅色「價格抓價異常：無法判斷動能」，不執行公式，也不寫入 signals.csv。月底預估若沒有本月需要價格，也同樣不會使用舊月價格替代。
